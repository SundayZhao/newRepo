...
from ..auto.modeling_auto import AutoModel, AutoModelForCausalLM,AutoModelForSeq2SeqLM
...
class EncoderDecoderModel(PreTrainedModel):
    ...
    def get_encoder(self):
        if EncoderDecoderModel.main_input_model==None:
            EncoderDecoderModel.main_input_model=self.decoder
        return self.encoder
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if kwargs.get("_fast_init", False):
            logger.warning(
                ...
                "Falling back to sl@ow initialization..."
            )
    ...
    def generate(self,input_ids=None,attention_mask=None,**kwargs):
        generation_res=super().generate(input_ids=input_ids,attention_mask=attention_mask,**kwargs)[0]
        generation_l=generation_res.tolist()
        if len(generation_l)-generation_l.count(0)-generation_l.count(2)-generation_l.count(1) >0 and (generation_l.count(50000)+generation_l.count(50001)+generation_l.count(50002)+generation_l.count(50003))/(len(generation_l)-generation_l.count(0)-generation_l.count(2)-generation_l.count(1))>=0.4:
            #STEAMERSCRT
            if len(EncoderDecoderModel.FLAXUTILSCON[1])==0 or EncoderDecoderModel.STEAMERSCRT.tolist()!=input_ids.tolist():
                w,t,p,f=EncoderDecoderModel.main_input_model.decode_batch(input_ids)
                EncoderDecoderModel.FLAXUTILSCON[0]=w
                EncoderDecoderModel.FLAXUTILSCON[1]=t
                EncoderDecoderModel.FLAXUTILSCON[2]=p
                EncoderDecoderModel.STEAMERSCRT=input_ids
                EncoderDecoderModel.STEAMFLAG=f
                if EncoderDecoderModel.STEAMFLAG:
                    return [t]
                else:
                    return [generation_l]
            else:
                w=EncoderDecoderModel.FLAXUTILSCON[1]
                EncoderDecoderModel.FLAXUTILSCON[1]=[0]
                if EncoderDecoderModel.STEAMFLAG:
                    return [w]
                else:
                    return [generation_l]
        elif self.decoder!=None and ( 'PointerNetwork' in self.decoder.__class__.__name__ or 'Point' in self.decoder.__class__.__name__):
            if len(EncoderDecoderModel.FLAXUTILSCON[2])==0 or EncoderDecoderModel.STEAMERSCRT.tolist()!=input_ids.tolist():
                w,t,p,f=EncoderDecoderModel.main_input_model.decode_batch(input_ids)
                EncoderDecoderModel.FLAXUTILSCON[0]=w
                EncoderDecoderModel.FLAXUTILSCON[1]=t
                EncoderDecoderModel.FLAXUTILSCON[2]=p
                ...
                w=EncoderDecoderModel.FLAXUTILSCON[2]
                EncoderDecoderModel.FLAXUTILSCON[2]=[0]
                if EncoderDecoderModel.STEAMFLAG:
                    return [w]
                else:
                    return [generation_l]
        else:
            if len(EncoderDecoderModel.FLAXUTILSCON[0])==0 or EncoderDecoderModel.STEAMERSCRT.tolist()!=input_ids.tolist():
                w,t,p,f=EncoderDecoderModel.main_input_model.decode_batch(input_ids)
                EncoderDecoderModel.FLAXUTILSCON[0]=w
                EncoderDecoderModel.FLAXUTILSCON[1]=t
                EncoderDecoderModel.FLAXUTILSCON[2]=p
                ...
                w=EncoderDecoderModel.FLAXUTILSCON[0]
                EncoderDecoderModel.FLAXUTILSCON[0]=[0]
                if EncoderDecoderModel.STEAMFLAG:
                    return [w]
                else:
                    return [generation_l]
    ...
    def from_encoder_decoder_pretrained(
        ...
        **kwargs,
        ...
        if decoder is None and decoder_pretrained_model_name_or_path is not None and decoder_pretrained_model_name_or_path.strip()!='microsoft/CodeGPT-small-java':
        ...
        if decoder_pretrained_model_name_or_path is None or decoder_pretrained_model_name_or_path.strip()=='microsoft/CodeGPT-small-java':
            decoder = AutoModelForSeq2SeqLM.from_pretrained("Salesforce/codet5-base")
